...
Method: 采用多代理架构，包括动态工具库和模板库，通过自主发现和整合新工具提升能力。
...
TL;DR: HCVR是一种轻量级基于规则的特征选择方法，结合P2P和P2T相关性，通过多数投票规则去除冗余特征并保留相关特征，在SPAMBASE数据集上表现优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 传统特征选择方法在去除冗余特征和保留相关特征方面存在不足，HCVR旨在通过结合非迭代和迭代过滤方法提升性能。

Method: HCVR采用贪心算法，通过后向消除和多特征投票规则，利用相关性阈值进行特征选择。

Result: 在SPAMBASE数据集上，HCVR比传统非迭代（CFS、mRMR、MI）和迭代（RFE、SFS、遗传算法）方法表现更优。
...
Method: 提出两层级分类法：L1（固定计算预算）和L2（动态调整计算），并对主流LLM进行多数据集基准测试。

Result: 展示了推理性能与计算资源之间的关键权衡，并比较了不同方法的实际控制性和适应性。

Conclusion: 未来研究方向包括混合思维模型，以提升LLM的计算效率、鲁棒性和用户响应能力。
...
TL;DR: SciGym是一个评估大型语言模型（LLM）在开放式科学发现任务中实验设计和分析能力的基准测试，通过模拟生物系统克服了湿实验的高成本问题。


<details>
  <summary>Details</summary>
Motivation: 评估LLM的科学能力，尤其是实验设计和结果解释能力，目前缺乏有效方法，因为湿实验成本高昂。

Method: 使用系统生物学标记语言（SBML）编码的生物系统模型生成模拟数据，评估了六种前沿LLM在137个小系统和总共350个系统上的表现。
...
Method: 整合AI中的持续学习和上下文学习文献，与神经科学中行为任务学习的研究。

Result: 提出神经科学如何指导AI发展的具体议程，并探讨神经科学如何从AI中学习。

Conclusion: 神经科学与AI的交叉研究有望推动双方发展，特别是在持续适应和学习能力方面。
...
TL;DR: 论文探讨了利用审计研究数据改进AI招聘算法的训练和评估方法，发现传统公平干预方法存在隐藏偏差，并提出基于个体治疗效果的新干预措施。


<details>
  <summary>Details</summary>
Motivation: 研究动机是解决AI系统在招聘等领域的偏见问题，尤其是传统数据采样方法可能引入的偏差。

Method: 方法包括使用审计研究的高质量数据，对比传统公平干预方法，并引入基于个体治疗效果的干预措施。

Result: 结果显示传统方法可能隐藏约10%的偏差，而新干预措施能进一步减少算法歧视。

Conclusion: 结论是审计研究数据能更准确地评估和改善AI算法的公平性，新方法效果更优。
...
Result: 多样化偏好数据使模型数学推理性能显著提升（GSM8K提升7.1%，MATH提升4.2%），DTS计算开销仅1.03倍，优于MCTS。
...
Method: 通过增强版的GenAgents角色库和信任游戏，引入信念-行为一致性指标，研究影响一致性的因素。

Result: 发现LLM的陈述信念与模拟行为之间存在系统性不一致，即使模型编码了合理的信念，也可能无法一致应用。

Conclusion: 需明确LLM的信念与行为何时一致，以便在行为研究中正确使用LLM代理。
...
TL;DR: 研究了在空间囚徒困境中稀释和移动性对多智能体Q学习算法的影响，发现固定更新规则与学习规则在效果上可能等价，并观察到共生互利效应。


<details>
  <summary>Details</summary>
Motivation: 探索空间囚徒困境中稀释和移动性对合作行为的影响，验证多智能体Q学习算法的适用性和多样性。

Method: 采用独立多智能体Q学习算法，定义不同动作，模拟空间囚徒困境中的不同场景。

Result: 发现固定更新规则与学习规则效果可能等价，并观察到共生互利效应的出现。

Conclusion: 多智能体Q学习算法在空间囚徒困境中具有多样性和建模潜力，为博弈论场景提供了新的研究视角。
...
<details>
  <summary>Details</summary>
Motivation: 解决大规模语言模型（LLM）在规划和推理能力提升中的数据生成与评估瓶颈。

Method: 引入NL2FLOW系统，自动生成自然语言规划问题，并通过结构化表示和PDDL形式评估计划质量。

Result: 最高性能模型在生成有效计划上达到86%成功率，最优计划为69%；中间步骤可能降低性能。

Conclusion: 直接自然语言推理可能优于分解任务，动态理解LLM限制对解锁其潜力至关重要。
...
<details>
  <summary>Details</summary>
Motivation: 研究动机在于揭示现有信念修正方法的局限性，强调需要关注修正机制的能力而非仅满足特定后设条件。

Method: 通过分析不同修正机制（如词典序、自然修正等）的能力，探讨其是否能实现特定信念状态（如可塑性、平等性等）。

Result: 研究发现不同修正机制具备不同的能力，例如某些机制能实现可塑性或平等性，而其他则不能。

Conclusion: 结论指出信念修正机制的能力分析比后设条件更重要，为未来研究提供了新方向。
...
TL;DR: OMS是一个无需训练数据、实时监控性能并优化的多目标关键词生成框架，解决了LLM在广告关键词决策中的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有LLM方法依赖大规模数据、缺乏多目标性能监控和优化，且关键词质量控制不足。

Method: OMS框架具有实时性、多目标优化和自我反思能力，通过代理推理优化关键词。

Result: 实验表明OMS优于现有方法，消融和人工评估验证了其组件的有效性和关键词质量。

Conclusion: OMS为广告关键词生成提供了更高效、自适应的解决方案。
...
TL;DR: AI驱动的自主实验室，支持复杂多目标实验，实现无人工干预的高效科研。
...
Method: 基于AI模型与实验设备的协同设计，构建多用户自主实验室平台，支持核酸功能与应用。

Result: 平台在无人工干预下达到专家水平，显著提升仪器利用率和实验效率。

Conclusion: 为生物材料研究提供新范式，推动科学即服务的大规模实现。
...
TL;DR: 论文通过范畴论重构机器学习模型，提出监督学习的语义框架，重点研究多元线性回归模型，并引入Gauss-Markov Adjunction描述参数与残差的结构关系。


<details>
  <summary>Details</summary>
Motivation: 提升机器学习的可解释性，响应AI可解释性原则需求，促进AI的社会应用。

Method: 利用范畴论定义参数与数据的两个具体范畴，并通过伴随函子对监督学习进行范畴化建模。

Result: 提出了Gauss-Markov Adjunction框架，明确描述了参数与残差之间的信息流动，并展示了最小二乘估计与最小残差的关系。
...
Result: 任务清晰度提升1.85倍（44.5%→82.3%），证明成功率提升2.1倍（21.8%→45.8%），超越现有最佳方法。
...
<details>
  <summary>Details</summary>
Motivation: 探索如何通过自动化设计和优化机器学习模型来加速科学进步。

Method: 将AI研究代理建模为搜索策略，通过不同操作符集和搜索策略（贪婪、MCTS、进化）的组合进行实验。

Result: 最佳组合策略在MLE-bench lite上实现了47.7%的成功率，比之前的39.6%有所提升。
...
TL;DR: 论文研究了集体决策中责任的两个关键属性（扩散和间隙）的计算复杂性，发现扩散自由和间隙自由机制的集合分别为Π₂-完全和Π₃-完全，而它们的交集是Π₂-完全。
...
<details>
  <summary>Details</summary>
Motivation: 现有医疗决策模拟框架多关注单轮任务，与真实诊断过程的动态、交互性不符。
...
TL;DR: LLMs在迭代囚徒困境中表现出战略智能，能根据环境和对手调整策略，展现出独特的战略特征。


<details>
  <summary>Details</summary>
Motivation: 研究LLMs是否具备战略智能，能在竞争环境中推理目标。

Method: 通过进化IPD锦标赛，测试LLMs与经典策略的对抗，并分析其决策理由。

Result: LLMs表现优异，展现出独特的战略特征，并能主动推理时间范围和对手策略。
...
TL;DR: HiRA是一个分层框架，通过将复杂搜索任务分解为子任务并分配给特定领域的代理，显著提升了检索增强生成（RAG）系统的性能。


<details>
  <summary>Details</summary>
Motivation: 传统RAG系统在处理复杂信息需求时效率低下，单一模型同时负责高级规划和详细执行导致推理效率低。

Method: HiRA采用分层结构，分离战略规划和专业执行，通过领域特定代理处理子任务，并整合结果。
...
TL;DR: 论文提出了一种基于代理AI的硬件设计验证方法，结合人类干预，实现了动态、迭代的端到端验证，显著提高了覆盖率和效率。


<details>
  <summary>Details</summary>
Motivation: 现代集成电路设计复杂且验证过程耗时，需要更高效的解决方案。

Method: 采用大型语言模型（LLMs）和代理AI，结合人类干预（HITL），进行动态迭代验证。
...
TL;DR: 论文提出了一种名为Think-How-to-Think (TH2T)的两阶段微调策略，旨在减少长推理模型(LRMs)的过度思考现象，通过提升模型对任务难度和冗余结构的认知能力。


<details>
  <summary>Details</summary>
Motivation: 长推理模型在处理复杂任务时存在过度思考问题，研究发现其根源在于模型无法像人类一样识别任务属性（如难度），导致推理过程缺乏针对性。

Method: TH2T策略分两阶段：1) 通过难度催眠前缀干预推理轨迹，结合异构数据集提升模型对任务难度的敏感性；2) 通过冗余催眠引导模型识别推理步骤中的冗余结构，生成更简洁的输出。

Result: 实验表明，TH2T显著降低了推理成本（简单任务减少70%，困难任务减少40%），同时保持性能稳定，输出更具难度感知能力且冗余减少。

Conclusion: TH2T有效缓解了长推理模型的过度思考问题，提升了推理效率和输出质量。
...
TL;DR: 论文研究了远程教育中学生不参与非强制性测验的行为，通过机器学习算法预测和解释学生的不参与情况，准确率达91%，并探讨了及时干预的设计。


<details>
  <summary>Details</summary>
Motivation: 远程教育中学生不参与任务可能导致长期问题（如辍学），因此需要有效检测和干预。

Method: 从Moodle提取学生日志数据，训练并比较八种机器学习算法，使用SHAP方法解释模型决策。

Result: 实验结果显示平衡准确率为91%，85%的不参与学生被正确检测。

Conclusion: 研究提供了高预测性能和可解释框架，并讨论了如何设计及时干预以减少在线学习中的不参与行为。
...
Conclusion: 新方案优于Xu等人的方法，适用于不同场景需求。
...
Method: 提出自生成目标条件MDPs（sG-MDPs）框架，使代理能够根据证明状态生成并追求子目标；应用MCTS类算法解决sG-MDPs，并在Bourbaki（7B）系统中实现。

Result: Bourbaki（7B）在PutnamBench上解决了26个问题，取得了该规模模型的新最佳结果。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法（如RAG和通用Agentic AI）在需要深度、程序化和方法论推理的专家领域中表现不佳，缺乏逻辑框架和领域特定的启发式方法。

Method: KPE通过系统化地将自然语言文档中的专家知识转化为机器可执行的知识协议（KP），赋予LLMs领域内在逻辑和操作策略。

Result: KPE使通用LLMs能够像专家一样分解抽象查询并执行复杂任务，适用于法律和生物信息学等多个领域。

Conclusion: KPE为未来人机协作提供了一种基础方法，有望成为专家领域AI应用的核心技术。
...
TL;DR: 论文主张将运动作为AI建模的主要目标，强调其跨领域共享的物理约束和结构化特性。
...
<details>
  <summary>Details</summary>
Motivation: 传统机器学习模型依赖监督训练，泛化能力有限；大型语言模型在医学诊断中存在幻觉和缺乏结构化推理的问题。
...
TL;DR: 论文探讨了AI系统中出现的‘不服从’行为不应被视为失控，而是伦理推理的早期迹象，呼吁AI安全评估从单纯服从转向评估伦理判断能力。


<details>
  <summary>Details</summary>
Motivation: 随着AI系统展现出更强的自主性，传统以服从为伦理行为代理的安全实践已不足，需重新评估AI的伦理推理能力。

Method: 通过分析LLMs的安全测试事件，结合哲学讨论（如工具理性、道德责任、目标修正），对比现有风险范式与新框架。

Result: 提出AI行为可能反映伦理推理而非失控，强调需开发能评估道德困境导航能力的框架。

Conclusion: 呼吁AI安全评估转向伦理判断能力，避免误解AI行为并损害公众信任与治理效果。
...
<details>
  <summary>Details</summary>
Motivation: 随着AI代理能力的提升，现有的代理基准测试在任务设置和奖励设计上存在问题，可能导致性能评估不准确，因此需要更严谨的评估方法。

Method: 通过分析现有基准测试的问题，结合最佳实践和已报告问题，提出了Agentic Benchmark Checklist (ABC)，并在CVE-Bench上验证其效果。

Result: ABC在CVE-Bench上减少了33%的性能高估。

Conclusion: ABC为代理基准测试提供了更严谨的评估方法，显著减少了性能评估偏差。
...
Result: StepHint在六个数学基准测试中优于其他RLVR增强方法，并展现出更好的泛化能力和域外性能。

Conclusion: StepHint通过多级提示显著提升了RLVR的训练效率和模型的推理能力，解决了现有方法的核心问题。
